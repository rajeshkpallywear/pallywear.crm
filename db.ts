...
export async function initDB() {
  ...
  try {
    ...
    try {
      await pool.execute("UPDATE tasks SET marketing_image = original_design_file WHERE (marketing_image IS NULL OR marketing_image = '') AND original_design_file IS NOT NULL AND original_design_file LIKE 'data:image/%'");
    } catch (_) {}
    ...
    const alterQueries = [
      ...
      "ALTER TABLE `orders` ADD COLUMN `marketing_notes` LONGTEXT DEFAULT NULL",
      "ALTER TABLE `orders` ADD COLUMN `productionNotes` LONGTEXT DEFAULT NULL",
      "ALTER TABLE `orders` ADD COLUMN `voiceNote` LONGTEXT DEFAULT NULL",
      "ALTER TABLE `orders` MODIFY COLUMN `notes` LONGTEXT DEFAULT NULL",
      "ALTER TABLE `orders` MODIFY COLUMN `marketing_notes` LONGTEXT DEFAULT NULL",
      "ALTER TABLE `orders` MODIFY COLUMN `productionNotes` LONGTEXT DEFAULT NULL",
